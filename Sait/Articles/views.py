...
def profile(request):  # страница профиля
    ...
    conference_a = "все мероприятия"
    conference_a += " >"
    events = Event.objects.all().filter(user=request.user)[:3]  # Нужно ограничить до 3 элементов
    # теги блока 3-last-article
    article_h2 = "Последние статьи"
    article_a = "все статьи"
    article_a += " >"
    articles = Article.objects.all().filter(user=request.user)[:3]  # Нужно ограничить до 3 элементов
